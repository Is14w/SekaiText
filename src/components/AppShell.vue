  ...
  <div class="min-h-screen bg-[var(--color-bg)] flex">
    ...
    >
      ...
      >
        ...
        <span v-if="sidebarOpen" class="text-xs font-medium whitespace-nowrap">导航</span>
      </button>
      ...
      <div class="flex flex-col gap-0.5 p-1.5">
        ...
        >
          ...
          <span v-if="!sidebarOpen" class="absolute left-full ml-2 px-2 py-0.5 rounded text-xs bg-gray-900 text-white whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
            {{ m.label }}
          </span>
        </button>
      </div>
      ...
      <!-- Settings Section -->
      <div v-if="sidebarOpen" class="flex-1 overflow-y-auto p-3 space-y-3 text-sm">
        <div class="text-xs font-medium text-[var(--color-text-secondary)] uppercase tracking-wider">设置</div>
        ...
        <label class="flex items-center justify-between cursor-pointer">
          ...
          <input v-model="settings.settings.saveN" type="checkbox" class="accent-[var(--color-primary)] w-3.5 h-3.5" />
        </label>
        ...
        <label class="flex items-center justify-between cursor-pointer">
          ...
          <input v-model="settings.settings.disableSSL" type="checkbox" class="accent-[var(--color-primary)] w-3.5 h-3.5" />
        </label>
      </div>
      ...
      <div class="border-t border-[var(--color-border)] p-1.5">
        ...
        >
          ...
          <span class="absolute left-full ml-2 px-2 py-0.5 rounded text-xs bg-gray-900 text-white whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity z-50">
            设置
          </span>
        </button>
        <button
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <div class="border-b border-[var(--color-border)] bg-[var(--color-surface)] px-4 py-1.5">
        <div class="flex items-center gap-2 flex-wrap text-sm">
          <button @click="handleOpen" class="px-2.5 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-[var(--color-text-secondary)] hover:text-[var(--color-text)] transition-colors">打开</button>
          <button @click="handleSave" class="px-2.5 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-[var(--color-text-secondary)] hover:text-[var(--color-text)] transition-colors">保存</button>
          <button @click="handleClear" class="px-2.5 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-[var(--color-text-secondary)] hover:text-[var(--color-text)] transition-colors">清空</button>
          ...
          <label class="flex items-center gap-1 cursor-pointer text-[var(--color-text-secondary)] hover:text-[var(--color-text)]">
            <input v-model="app.showFlashback" type="checkbox" class="accent-[var(--color-primary)] w-3 h-3" />
            闪回
          </label>
          <label class="flex items-center gap-1 cursor-pointer text-[var(--color-text-secondary)] hover:text-[var(--color-text)]">
            <input v-model="app.syncScroll" type="checkbox" class="accent-[var(--color-primary)] w-3 h-3" />
            同步
          </label>
          <label class="flex items-center gap-1 cursor-pointer text-[var(--color-text-secondary)] hover:text-[var(--color-text)]">
            <input v-model="app.showDiff" type="checkbox" class="accent-[var(--color-primary)] w-3 h-3" />
            差异
          </label>

          <div class="w-px h-4 bg-[var(--color-border)]" />

          <button @click="showSpeakerCheck = true" class="px-2.5 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-[var(--color-text-secondary)] hover:text-[var(--color-text)] transition-colors">说话人</button>
          <button @click="handleFullCheck" class="px-2.5 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-[var(--color-text-secondary)] hover:text-[var(--color-text)] transition-colors">检查</button>
          <button @click="showSpeakerCount = true" class="px-2.5 py-1 rounded hover:bg-gray-100 dark:hover:bg-gray-800 text-[var(--color-text-secondary)] hover:text-[var(--color-text)] transition-colors">统计</button>
        </div>
      </div>
